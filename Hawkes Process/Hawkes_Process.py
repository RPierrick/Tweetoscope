...
#------ Fonctions pour lire le topics "cascades_series" -------#

def identifiant_tweet(string):
    liste = string.split("'")
    identifiant = liste[4][6:-3]
    return(identifiant)
...
def time_series(string):
    ...
    one = string.split("'")
    two = one[6]
    doublet = two[10:-2].split(';') # ['(t1,m1)',...] => [[t1,m1],...]
    for elem in doublet:
        a = re.sub('[()]','',elem)
        b = a.split(',')
        series.append([float(b[0]),float(b[1])])
...
for record in consumer:
    cascade_series = record.value.decode()
    tweet_id = identifiant_tweet(cascade_series)
    Series = time_series(cascade_series)
    t = Series[-1,0]
    n_obs = len(Series)
    res = MAP(Series,t,alpha,mu)
    p_est,beta_est = res[1]
    N = prediction((p_est,beta_est), Series, alpha, mu, t)
    message = {'type ' : 'parameters','cid ' : tweet_id,'n_obs ' : n_obs,'n_supp ' : N, 'p ' : p_est,'beta ' : beta_est}
